...
Instructor Name: Professor Gutekunst
The example is from 
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        #check what the user chose from the pulldown menu
        s='?'
        
        if data["pulldown"]=="Compute Inventory":
            s=inventory.compute_inventory()
            
            
        elif data["pulldown"]=="List items":
            if data["line"]:
                x=data["line"].split(",")
                s=inventory.print_inventory(int(x[0]),int(x[1]))
            else:
                s=inventory.print_inventory()
                print(s)
                
        elif data["pulldown"]=="Search by pattern":
            s=inventory.search_item(data["line"])
            
        
        if s!='?':
            ret_text = home_page + '\n' + 'Your inputs are as follows.<p>' \
                  +s+ '<p>' + home_url + '<p>'
        else:
            ret_text = home_page + '\n' + 'Your inputs are as follows.<p>' \
                  + str(data) + '<p>' + home_url + '<p>'
